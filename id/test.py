...
def parse_arg():
    ...
    parser.add_argument('--image_path', type=str, default='id', help='the path to your image')
    parser.add_argument('--checkpoint', type=str, default='model/id.pth',
...
if __name__ == '__main__':
    ...
    started = time.time()
    d=0
    for x in glob.glob(args.image_path+"/*"):
        img0 = cv2.imread(x)
        cv2.imshow("image",img0)
        img = cv2.cvtColor(img0, cv2.COLOR_BGR2GRAY)
        converter = utils.strLabelConverter(config.DATASET.ALPHABETS)

        text=recognition(config, img, model, converter, device)
        cv2.imwrite("output/op"+str(d)+"_"+text+".jpg",img0)
        d+=1
        finished = time.time()
